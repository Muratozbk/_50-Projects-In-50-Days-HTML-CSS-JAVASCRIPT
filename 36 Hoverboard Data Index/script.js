...
function setColor(sqr) {
    ...
    sqr.style.backgroundColor = color
    sqr.style.boxShadow = `0 0 2px ${color}`
}
...
for (let i = 0; i < colors.length; i++) {
    ...
    squares.setAttribute('data-index', i)

    allColors.appendChild(squares)
}

allColors.addEventListener('click', (e) => {
    const square = e.target; {
        if (square.classList.contains('square')) {
            const idx = square.getAttribute('data-index');
            colors.splice(idx, 1)
            square.remove()
            console.log(square, colors)
            allColors.querySelectorAll('.square').forEach((sqr, idx) => {
                sqr.setAttribute('data-index', idx)
            })
        }
    }
})
